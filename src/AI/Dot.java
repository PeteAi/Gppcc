...
public class Dot {
    ...
    void show(Graphics g) {
        ...
        if (isBest) {
            ...
            g.drawOval((int) pos.x, (int) pos.y, 8, 8);
        } else {//all other dots are just smaller black dots
            g.setColor(Color.BLACK);
            g.drawOval((int) pos.x, (int) pos.y, 4, 4);
        }
    }
    ...
    void update() {
        if (!dead && !reachedGoal) {
            move();
            if (pos.x < 2 || pos.y < 2 || pos.x > Host.width - 2 || pos.y > Host.height - 2) {//if near the edges of the window then kill it
                dead = true;
            } else if (Methods.dist(pos.x, pos.y, Host.goal.x, Host.goal.y) < 5) {//if reached goal
    ...
    void calculateFitness() {
        if (reachedGoal) {//if the dot reached the goal then the fitness is based on the amount of steps it took to get there
            ...
            float distanceToGoal = Methods.dist(pos.x, pos.y, Host.goal.x, Host.goal.y);
    ...
    Dot gimmeBaby() {
        Dot baby = new Dot();
        baby.brain = brain.cloneBrain();//babies have the same brain as their parents
        return baby;
    }
}
